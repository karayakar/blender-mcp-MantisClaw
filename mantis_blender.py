# Code created by Karay Akar @ https://github.com/karayakar/blender-mcp-MantisClaw
...
bl_info = {
    "name": "MantisClaw_Blender MCP",
    "author": "Karay Akar @ https://github.com/karayakar/blender-mcp-MantisClaw",
    "version": (1, 2),
    "blender": (3, 0, 0),
    "location": "View3D > Sidebar > MantisClaw_Blender MCP",
    "description": "Connect MantisClaw_Blender MCP via MCP",
    "category": "Interface",
}
...
class BlenderMCPServer:
    ...
    def list_objects(self, limit=200, object_type=None, include_hidden=True):
        """List scene objects with basic transform information."""
        if limit is None:
            limit = 200
    ...
    def set_object_transform(self, name, location=None, rotation=None, scale=None, relative=False):
        """Set or offset object transform values."""
        obj = bpy.data.objects.get(name)
        if not obj:
            raise ValueError(f"Object not found: {name}")

        def _coerce_vec3(value, field_name):
            if value is None:
                return None
            if not isinstance(value, (list, tuple)) or len(value) != 3:
                raise ValueError(f"{field_name} must be a list/tuple with 3 numbers")
            return (float(value[0]), float(value[1]), float(value[2]))

        loc = _coerce_vec3(location, "location")
        rot = _coerce_vec3(rotation, "rotation")
        scl = _coerce_vec3(scale, "scale")

        if loc is None and rot is None and scl is None:
            raise ValueError("At least one of location, rotation, scale must be provided")
    ...
    def create_primitive(
        ...
        """Create a mesh primitive and return object info."""
        primitive_key = str(primitive).lower()
        ...
        if primitive_key == "cube":
            bpy.ops.mesh.primitive_cube_add(size=float(size), location=location, rotation=rotation)
        elif primitive_key == "uv_sphere":
            bpy.ops.mesh.primitive_uv_sphere_add(radius=float(radius), location=location, rotation=rotation)
        elif primitive_key == "ico_sphere":
            bpy.ops.mesh.primitive_ico_sphere_add(radius=float(radius), location=location, rotation=rotation)
        elif primitive_key == "cylinder":
            bpy.ops.mesh.primitive_cylinder_add(radius=float(radius), depth=float(depth), location=location, rotation=rotation)
        elif primitive_key == "cone":
            bpy.ops.mesh.primitive_cone_add(radius1=float(radius), depth=float(depth), location=location, rotation=rotation)
        elif primitive_key == "plane":
            bpy.ops.mesh.primitive_plane_add(size=float(size), location=location, rotation=rotation)
        elif primitive_key == "torus":
            bpy.ops.mesh.primitive_torus_add(location=location, rotation=rotation)
        else:
            raise ValueError("Unsupported primitive. Use cube, uv_sphere, ico_sphere, cylinder, cone, plane, or torus")

        obj = bpy.context.active_object
        if obj is None:
            raise RuntimeError("Primitive creation did not return an active object")
        ...
        if scale:
            if not isinstance(scale, (list, tuple)) or len(scale) != 3:
                raise ValueError("scale must be a list/tuple with 3 numbers")
